...
How the four layouts are built, how to add a fifth, and the one thing about the preview that has cost
...
own default rather than carrying over a choice made for another. Modern additionally flips its own
text between ink and white depending on how dark the picked colour is, so the panel stays readable at
either end.
...
The panel takes over the editing column rather than floating over the sheet: four thumbnails want to
